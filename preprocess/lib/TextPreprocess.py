...
class TextPreprocess:
    ...
    def process_to_class(self,
        ...
        for s in content_list:
            ...
            c_content_list.append(to_tokenizer)
        content_list = c_content_list
        ...
        if self.spanish_tokenizer is not None:
            sentences = self.spanish_tokenizer.tokenize(' '.join(content_list))
            for s in sentences:
                t_content_list.extend([s for s in word_tokenize(s)])
        else:
            token_list = [s for s in word_tokenize(' '.join(content_list))]
            t_content_list = token_list

        # filter token in model vocabulary list
        if vocab or vocab_path is not None:
            t_content_list = self.model_vocab_filter(
                token_list=t_content_list,
                vocab=vocab,
    ...
    def process_to_clean(self, doc: str):
        # clean punctuation marks and non-words text
        content = self.load_document(doc)
        ...
        content_list = list()
        if self.spanish_tokenizer is not None:
            sentence_list = [s for s in self.spanish_tokenizer.tokenize(content)]
            for s in sentence_list:
                to_tokenizer = self.flat_text(s, keep_sentence=True)
                content_list.extend([t for t in word_tokenize(to_tokenizer)])
        else:
            content_list = content.split()
    ...
    @staticmethod
    def flat_text(text: str, keep_sentence=False):
        if text is None:
            return ''
        ...
        if keep_sentence:
            ...
            new_ = re.sub(r'\n', ' ', new_)
        new_ = re.sub(r'\\n', ' ', new_)
        new_ = re.sub(r'\.\s{0,100}', '. ', new_)
        new_ = re.sub(r'\s{0,100}\.\s', '.', new_)
        new_ = re.sub(r'\t', ' ', new_)
        new_ = re.sub(r'\s{2,1000}', ' ', new_)
    ...
    def simplify(text: str):
        clean_text = text
        try:
            chars = [c for c in unicodedata.normalize('NFKC', text)
                     if c not in string.punctuation]
            if chars:
                clean_text = unicodedata.normalize('NFKC', ''.join(chars))
    ...
    def process_vocabulary_from_file(self,
        ...
        print(f"Total TXT documents: {total}")
        pool_iter = total // min(total, 100)
        pool_iter_list = list()
        for n, pool_content_file_list in enumerate(
        ...
        with Pool() as pool:
            # call the same function with different data in parallel
            for result in pool.starmap(self._process_vocabulary_from, pool_iter_list):
                pre_process_list.extend(result)
    ...
    def _process_vocabulary_from(self,
        ...
        for n, content in enumerate(content_list, 1):
            print(f"Pre-processing pool [{process_id}/{n}/{total}]", end='')
            try:
                full_text = self.load_document(content)
                token_list = self.process_to_class(
                    full_text, stem=stem, is_spanish=is_spanish, token_filter=token_filter, greater=greater)
                if token_list:
